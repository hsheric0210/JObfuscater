...
public class SourceFileObfuscation extends Transformer
{
	public enum Mode
	{
		Remove("Removed"),
	...
	{
		public final SourceFileObfuscation.Mode mode;
		public final String replaceTo;
		public final StringGenerator nameGenerator;

		public Parameter(final SourceFileObfuscation.Mode mode, final String value, final StringGenerator nameGenerator)
		{
			this.mode = mode;
	...
	public SourceFileObfuscation(final Parameter param)
	{
	...
	public final void transform()
	{
		...
		{
			classWrapper.classNode.sourceFile = this.parameter.mode == Mode.Remove ? null : this.parameter.mode == Mode.Randomize ? this.parameter.nameGenerator.generate() : this.parameter.replaceTo;;
			affects.incrementAndGet();
		});
		Logger.stdOut(String.format("%s %d SourceFile attributes" + (this.parameter.mode == Mode.Replace && this.parameter.replaceTo.length() < 32 ? " to \"" + this.parameter.replaceTo + '"' : "") + '.', this.parameter.mode.resultMessage, affects.get()));
	}
